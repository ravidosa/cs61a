...
class Transaction:
    ...
    def report(self):
        ...
        msg = 'no change'
        if self.changed():
            msg = f"{'decreased' if self.before > self.after else 'increased'} {self.before}->{self.after}"
...
def make_change(amount, coins):
    ...
    if amount == smallest:
        return [smallest]
    used_smallest = make_change(amount - smallest, rest)
    unused_smallest = make_change(amount, rest)
    if used_smallest is None and unused_smallest is None:
        return None
    elif used_smallest is None:
        return unused_smallest
    else:
        return [smallest] + used_smallest
