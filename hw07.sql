...
CREATE TABLE siblings AS
  SELECT a.child AS younger, b.child AS older
  FROM parents AS a, parents AS b
  WHERE a.parent = b.parent AND a.child < b.child;

-- Sentences about siblings that are the same size
CREATE TABLE sentences AS
  SELECT ("The two siblings, " || siblings.younger || " and " || siblings.older || ", have the same size: " || a.size)
  FROM siblings, size_of_dogs AS a, size_of_dogs AS b
  WHERE siblings.younger = a.name AND siblings.older = b.name and a.size = b.size;
