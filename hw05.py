...
class Player:
    ...
    def debate(self, other):
        ...
        prob = self.random_func()
        self.popularity += (50 if prob < threshold else -50)
        self.popularity = max(self.popularity, 0)
...
class Game:
    ...
    def play(self):
        while not self.game_over():
            if self.turn % 2 == 0:
                curr, opp = self.p1, self.p2
            else:
                curr, opp = self.p2, self.p1
            curr.choose(opp)(opp)
            self.turn += 1
        return self.winner()
    ...
    def winner(self):
        return self.p1 if self.p1.votes > self.p2.votes else self.p2 if self.p1.votes < self.p2.votes else None
...
def store_digits(n):
    ...
    def digit(d, rem):
        if d == 0:
            return rem
        return digit(d // 10, Link(d % 10, rem))
    return digit(n, Link.empty)
...
def crispr_gene_insertion(lnk_of_genes, insert):
    ...
    while lnk_of_genes is not Link.empty:
        insert_gene(lnk_of_genes.first, depth)
