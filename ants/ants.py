...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        check_pos = 0
        while not check_place.is_hive and check_pos < self.lower_bound:
            check_place = check_place.entrance
            check_pos += 1
        while not check_place.is_hive and check_pos <= self.upper_bound:
            if check_place.bees:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        for bee in self.place.bees.copy():
            bee.reduce_health(amount + (self.health <= amount) * self.damage)
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while check_place is not None:
            if check_place.ant is not None:
                if check_place.ant.is_container and check_place.ant.ant_contained:
                    check_place.ant.ant_contained.double()
                check_place.ant.double()
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        while not check_place.is_hive:
            if check_place.ant is not self and check_place.ant is not None:
                insect_dict[check_place.ant] = dist
            for bee in check_place.bees.copy():
                insect_dict[bee] = dist
            check_place = check_place.entrance
            dist += 1
        return insect_dict
        # END Problem EC 4
    ...
    def calculate_damage(self, distance):
        damage_decay = 0.0625
        dist_decay = 0.25
        return max(0, self.damage - damage_decay * self.insects_shot - dist_decay * distance)
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        gamestate -- The GameState, used to access game state information.
        """
        if self.scared and self.scare_countdown:
    ...
    def set_slow_countdown(self, length):
